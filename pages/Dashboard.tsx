...
const handleVoiceSim = () => {
  // Check browser compatibility (Chrome and Edge fully support this)
  const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  
  if (!SpeechRecognition) {
    setToast({ message: "Speech recognition not supported in this browser.", icon: AlertTriangle });
    return;
  }

  const recognition = new SpeechRecognition();
  recognition.continuous = false; // Stop listening automatically when you stop speaking
  recognition.lang = 'en-US';    // Set the input language pattern

  // 1. Trigger when the microphone starts listening
  recognition.onstart = () => {
    setIsVoiceActive(true);
    setVoiceSearchText('Listening...');
  };

  // 2. Handle errors (like user denying microphone access permissions)
  recognition.onerror = (event) => {
    console.error("Speech Error:", event.error);
    setIsVoiceActive(false);
    setToast({ message: `Voice error: ${event.error}`, icon: MicOff });
  };

  // 3. Process the audio stream when you finish speaking
  recognition.onresult = (event) => {
    const transcript = event.results[0][0].transcript;
    setVoiceSearchText(transcript); // Puts your real spoken words on the screen

    // Evaluate the matching criteria from the voice transcription text string
    const lowerText = transcript.toLowerCase();

    if (lowerText.includes('airport') || lowerText.includes('lounge') || lowerText.includes('travel')) {
      setUserContext({ ...userContext, location: 'airport' });
      setSortBy('relevance');
      setActiveTab('all');
      setActiveCategory('travel');
      setToast({ message: "Switched to travel catalog via voice!", icon: Mic });
    } else {
      setToast({ message: `Searched for: "${transcript}"`, icon: Search });
    }
  };

  // 4. Reset UI assets once processing lifecycle ends completely
  recognition.onend = () => {
    setIsVoiceActive(false);
  };

  // Start the microphone capturing interface pipeline
  recognition.start();
};
